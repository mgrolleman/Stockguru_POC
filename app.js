(function(){
  ...
  var STRINGS = {
    en: {
      ...
      'playdata.assumptions': '"Buy once" assumes a single lump-sum purchase made on your chosen buy date, held until your chosen sell date. "Buy monthly (DCA)" means dollar-cost averaging — investing a fixed amount every month across that same period instead of all at once, something many beginner investors do (as covered in the quiz), since it smooths out the effect of timing. The examples above use \u20AC1,000 as a placeholder — since we\'re looking at percentage return, the actual amount you\'d invest doesn\'t change the result.',
      ...
      'playdata.resultLump': 'A single amount invested in {asset} in {buyMonth}, held until {sellMonth}, would be {dir} {pct}%.',
      'playdata.resultDca': 'Investing \u20AC1,000 per month into {asset} from {buyMonth} to {sellMonth} would be {dir} {pct}%.',
      'playdata.up': 'up',
      'playdata.down': 'down',
    ...
    nl: {
      ...
      'playdata.resultLump': 'Een eenmalig bedrag belegd in {asset} in {buyMonth}, aangehouden tot {sellMonth}, zou {pct}% {dir} staan.',
      'playdata.resultDca': '\u20AC1.000 per maand beleggen in {asset} van {buyMonth} tot {sellMonth} zou {pct}% {dir} staan.',
      'playdata.up': 'hoger',
      'playdata.down': 'lager',
  ...
  function renderSPXChart(){
    ...
    renderChartInto('netflix-chart', NFLX_DATA, '#C7681F', '#F5E3D3');
  ...
  var WHATIF_ASSETS = {
    ...
    nflx: { key:'nflx', data: NFLX_DATA, label:'Netflix', color:'#C7681F', fill:'#F5E3D3' }
  };
  ...
  function updateWhatIf(){
    ...
    if(!assetEl || !strategyEl || !chartEl || !resultEl || !buyM || !buyY || !sellM || !sellY) return;
    ...
    var slice = asset.data.slice(buyIdx, sellIdx + 1);
    ...
    var pctClass = returnPct >= 0 ? '' : ' neg';
    var pctSign = returnPct >= 0 ? '+' : '-';
    resultEl.className = 'trade-result-highlight' + pctClass;
    resultEl.innerHTML =
      '<div class="trade-result-pct">' + pctSign + Math.abs(returnPct).toFixed(1) + '%</div>' +
      '<div class="trade-result-desc">' + resultText + '</div>';
  }

  ['whatif-asset','whatif-strategy','trade-buy-month-select','trade-buy-year-select','trade-sell-month-select','trade-sell-year-select'].forEach(function(id){
