(function(){
  ...
  function updateHomeIntro(){
    ...
    if(playdataGoals.length){
      notes.push("Curious about risk vs. return? Check the Play with data tab for real examples.");
    }
    if(unmapped.length){
  ...
  function renderFeedbackSummary(data){
    ...
    var maxCount = Math.max.apply(null, data.topics.map(function(t){ return t.count; }));
    els.feedbackSummaryBars.innerHTML = data.topics.map(function(t){
      var widthPct = maxCount ? Math.round((t.count/maxCount)*100) : 0;
      return (
        ...
        '<div class="summary-bar-count">'+t.count+'</div>' +
        '</div>'
      );
    }).join('');
