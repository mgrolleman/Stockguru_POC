(function(){
  ...
  var STRINGS = {
    en: {
      ...
      'welcome.tagline': "The best app that combines an educational quiz and playing with real data!",
      ...
      'playdata.btcIntro': 'Bitcoin is a cryptocurrency with no underlying company or government, whose value rests entirely on supply, demand, and belief.',
      ...
      'playdata.step2': 'Step 2: choose a time horizon — how far back you want to look',
      ...
      'playdata.tradeLogTitle': 'Calculate your return',
      'playdata.tradeLogIntro': 'Pick a buy month and a sell month for the asset selected above, and see the actual return.',
      'playdata.buyLabel': 'Bought in',
      'playdata.sellLabel': 'Sold in (default: today)',
      'playdata.calculateBtn': 'Calculate return',
      'playdata.tradeResult': 'Buying {asset} in {buyMonth} and selling in {sellMonth} would have returned {dir} {pct}%.',
    ...
    nl: {
      ...
      'welcome.tagline': 'De beste app die een educatieve quiz en spelen met echte data combineert!',
      ...
      'playdata.btcIntro': 'Bitcoin is een cryptovaluta zonder onderliggend bedrijf of overheid, waarvan de waarde volledig steunt op vraag, aanbod en vertrouwen.',
      ...
      'playdata.step2': 'Stap 2: kies een tijdhorizon — hoever je terug wilt kijken',
      ...
      'playdata.tradeLogTitle': 'Bereken je rendement',
      'playdata.tradeLogIntro': 'Kies een koopmaand en een verkoopmaand voor de hierboven geselecteerde belegging, en bekijk het werkelijke rendement.',
      'playdata.buyLabel': 'Gekocht in',
      'playdata.sellLabel': 'Verkocht in (standaard: vandaag)',
      'playdata.calculateBtn': 'Bereken rendement',
      'playdata.tradeResult': '{asset} kopen in {buyMonth} en verkopen in {sellMonth} had een rendement van {pct}% {dir} opgeleverd.',
  ...
  function renderFeedbackSummary(data){
    ...
    els.feedbackSummaryBars.innerHTML = orderedTopics.map(function(t){
      ...
      if(isDummy){
        ...
        countLabel = Math.round((t.value/data.totalSubmissions)*100) + '%';
      } else {
        countLabel = widthPct + '%';
      }
  ...
  function monthInputToIndex(value){
    if(!value) return null;
    var parts = value.split('-');
    var y = parseInt(parts[0], 10), m = parseInt(parts[1], 10);
    var idx = (y - 2021) * 12 + (m - 8);
    if(idx < 0) idx = 0;
    if(idx > 59) idx = 59;
    return idx;
  }
  ...
  function calculateRealReturn(){
    ...
    var buyInput = document.getElementById('trade-buy-month');
    var sellInput = document.getElementById('trade-sell-month');
    if(!noteEl || !buyInput || !sellInput || !buyInput.value || !sellInput.value) return;

    var buyIdx = monthInputToIndex(buyInput.value);
    var sellIdx = monthInputToIndex(sellInput.value);

    if(sellIdx < buyIdx){
      noteEl.textContent = t('playdata.tradeInvalidRange');
    ...
    var buyPrice = asset.data[buyIdx].value;
    var sellPrice = asset.data[sellIdx].value;
    var pct = ((sellPrice / buyPrice) - 1) * 100;

    noteEl.textContent = t('playdata.tradeResult', {
      asset: whatifAssetLabel(asset),
      buyMonth: formatMonthInput(buyInput.value),
      sellMonth: formatMonthInput(sellInput.value),
      dir: pct >= 0 ? t('playdata.up') : t('playdata.down'),
      pct: Math.abs(pct).toFixed(1)
    });
    noteEl.classList.remove('hidden');
  }

  var tradeCalcBtn = document.getElementById('trade-calc-btn');
  var tradeBuyMonth = document.getElementById('trade-buy-month');
  var tradeSellMonth = document.getElementById('trade-sell-month');
  if(tradeCalcBtn) tradeCalcBtn.addEventListener('click', calculateRealReturn);
  if(tradeBuyMonth) tradeBuyMonth.addEventListener('change', calculateRealReturn);
  if(tradeSellMonth) tradeSellMonth.addEventListener('change', calculateRealReturn);
