(function(){
  ...
  function updateWhatIf(){
    ...
    currentWhatifSlice = slice;

    chartEl.innerHTML = buildChartSVG(slice, asset.color, asset.fill);
    attachChartInteraction('whatif-chart', function(){ return currentWhatifSlice; }, asset.color);
  ...
  function calculateRealReturn(){
    ...
    if(!noteEl || !buyM || !buyY || !sellM || !sellY) return;
    ...
    if(sellIdx < buyIdx){
      noteEl.className = 'trade-result-highlight neg';
      noteEl.innerHTML = '<div class="trade-result-desc">' + t('playdata.tradeInvalidRange') + '</div>';
    ...
    var desc = t('playdata.tradeResult', {
      asset: whatifAssetLabel(asset),
      buyMonth: asset.data[buyIdx].label,
      sellMonth: asset.data[sellIdx].label,
      dir: isPos ? t('playdata.up') : t('playdata.down'),
      pct: Math.abs(pct).toFixed(1)
    });

    noteEl.className = 'trade-result-highlight' + (isPos ? '' : ' neg');
    noteEl.innerHTML =
      '<div class="trade-result-pct">' + (isPos ? '+' : '-') + Math.abs(pct).toFixed(1) + '%</div>' +
      '<div class="trade-result-desc">' + desc + '</div>';
    noteEl.classList.remove('hidden');
  }
  ...
  ['trade-buy-month-select','trade-buy-year-select','trade-sell-month-select','trade-sell-year-select'].forEach(function(id){
    var el = document.getElementById(id);
    if(el) el.addEventListener('change', calculateRealReturn);
  });
